# 示例 📝

本文档提供了 CryptoPay Php SDK 的使用示例，包括 Demo 运行、密钥生成和回调处理。

## 1 SDK 实例对象 🛠️

### 1.1 所需配置 ⚙️

1. 注册您的业务名称并获取 `ApiKey` 和 `ApiSecret`；

2. 生成您自己的 `RSA` 密钥对；

3. 准备平台的 `RSA` 公钥；

### 1.2 创建签名对象 🔏

1. 添加配置文件 `config.yaml`。

```yaml
# 配置业务信息
ApiKey: ""
ApiSecret: ""
# 平台公钥
PlatformPubKey: ""
# 用于阻塞平台的公钥
PlatformRiskPubKey: ""
# 您自己的私钥
RsaPrivateKey: ""
```

2. 加载配置文件并创建 API 对象。

```php

	$config = [
		'key' => 'dkhl346iwonfw436',
		'secret' => '11e15f2d36f4e61c8a46fd426ae2189bc0406c1e37c29d8b136f75268a1d4216',
		'public_key' => '',
		'private_key' => '',
		'chain_public_key' => '',
	];
    global $config;
    $cryptoPay = new CryptoPay($config);

```

### 1.3 创建并签名请求数据。 ✍️

以用户创建为例。

```php

  // ....
	
    $open_id = 'project1757065502';

    $result = $cryptoPay->createUser($open_id);
     public function post($url, $data) {
        $sign = $this->sign($data);
        $clientSign = $this->encryption($data);

```

### 1.4 填充并发起请求 🚀

```php
      public function post($url, $data){
        $sign = $this->sign($data);
        $clientSign = $this->encryption($data);
        $header = [
            "key:{$this->config['key']}",
            "sign:{$sign}",
            "clientSign:{$clientSign}",
            "Content-Type:application/json",
            "timestamp:{$this->timestamp}",
        ];
        $curl = curl_init(); //初始化
        curl_setopt($curl, CURLOPT_URL,$url);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, FALSE);
        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, FALSE);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl, CURLOPT_POST, 1);
        curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($data));
        curl_setopt($curl, CURLOPT_HEADER,0);
        curl_setopt ($curl, CURLOPT_HTTPHEADER, $header);
        $result = curl_exec($curl);
        if (curl_errno($curl)) {
            print curl_error($curl);
        }
        curl_close($curl);

        return $result;
    }

```

### 1.5 验证解析返回数据 ✅

```php

	public function checkSignature($data, $sign)
    {
        $toSign = self::getSignString($data);
        $publicKeyId = openssl_pkey_get_public($this->config['chain_public_key']);
        $result = openssl_verify($toSign, base64_decode($sign), $publicKeyId, OPENSSL_ALGO_MD5);
        // openssl_free_key($publicKeyId);
        return $result === 1 ? true : false;
    }

```

## 2. 调用命令 📞

### 2.1. 注册新用户 🆕


进入 SDK 的 pay_sdk_php/ 目录，修改 $open_id 变量。

然后运行 php  example/create_user.php 来在平台上注册一个新用户。

如果尝试注册一个已经注册过的 open_id，将会返回错误。


### 2.2. 钱包注册 💼

进入 SDK 的 pay_sdk_php/ 目录，修改 $open_id 和 $chain_id变量。

然后运行 php  example/create_wallet.php 来完成用户在平台上的钱包注册。

### 2.3. 获取充值地址 📍

进入 SDK 的 pay_sdk_php/ 目录，修改 $open_id 和 $chain_ids 变量。

然后运行 php  example/get_wallet_addresses.php

### 2.4. 提现 💸

进入 SDK 的 pay_sdk_php/ 目录，修改 $open_id, $token_id, $amount, $address, $callback_url(可选), $sn(可选) 变量。

然后运行 php  example/withdraw.php
